...
pub struct Options {
    ...
    /// 导航 URL 覆盖（测试注入 fixture；生产恒为 CREATE_PAGE_URL）。
...
pub async fn run(
    ...
    let selectors = opts
        .selectors
        .clone()
        .unwrap_or_else(|| DEFAULT_SELECTORS.clone());
    ...
    let result = run_inner(account_name, &page, &selectors, &opts).await;
...
async fn run_inner(
    ...
    let mut flow = PublishPage::new(page, selectors);
    flow.require_logged_in().await?;
    flow.wait_home_ready(Duration::from_secs(60)).await?;
    ...
    flow.enter_create_page(Duration::from_secs(60)).await?;

    // 1) 上传
    werr(stderr, format_args!("步骤 1/5：上传视频…\n"));
    flow.upload_video(&opts.video).await?;
    flow.require_logged_in().await?;

    // 2) 元信息
    werr(stderr, format_args!("步骤 2/5：填写标题/描述/标签…\n"));
    flow.fill_title(&opts.title).await?;
    flow.fill_description(&opts.description).await?;
    flow.fill_tags(&opts.tags).await?;

    // 3) 封面
    if let Some(cover) = &opts.cover {
        werr(stderr, format_args!("步骤 3/5：设置封面…\n"));
        flow.set_cover(cover).await?;
    }

    // 4) 声明（默认保守不勾原创）
    werr(stderr, format_args!("步骤 4/5：核对声明选项…\n"));
    flow.check_declaration_conservative().await?;
